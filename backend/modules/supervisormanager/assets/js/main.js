function SupervisorManager()
{
    ...
    this.supervisorControl = function(event)
    {
        ...
        $.post(supervisorManagerConfig.urls.supervisorControl, {
    ...
    this.processControl = function(event)
    {
        ...
        $.post(supervisorManagerConfig.urls.processControl, {
    ...
    {
        var actionUrl = supervisorManagerConfig.urls.groupControl;

        if ($(event.currentTarget).hasClass('processConfigControl')) {
            actionUrl = supervisorManagerConfig.urls.processConfigControl;
        }
    ...
    this.groupProcessDelete = function(event)
    {
        ...
        $.post(supervisorManagerConfig.urls.countGroupProcesses, {
            groupName: groupName
        ...
        function call(actionType) {
            $.post(supervisorManagerConfig.urls.processConfigControl, {
                actionType: actionType,
                groupName : groupName
            }, responseHandler);
        }
    ...
    this.showLog = function(event)
    {
        ...
        $.post(supervisorManagerConfig.urls.getProcessLog, {
            processName: processName,
            logType: logType
        }, function(response) {
