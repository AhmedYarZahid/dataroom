                ...
                <div class="box-body">
                    ...
                    <?= $form->field($model, !empty($isMultipleMode) ? 'filePath[]' : 'filePath')->widget(FileInput::classname(), [
                        ...
                        'pluginOptions' => [
                            ...
                            'showCaption' => false,
                            'browseClass' => 'btn btn-primary btn-block mt-2',
                    ...
                    //     ? Html::a(!empty($isMultipleMode) ? Yii::t('admin', 'Switch to single mode') : Yii::t('admin', 'Switch to multiple mode'), [!empty($isMultipleMode) ? 'create-document' : 'create-multiple-documents', 'id' => $this->context->detailedRoomModel->id], ['class' => !empty($isMultipleMode) ? 'btn btn-xs btn-warning pull-right ' : 'btn btn-xs btn-warning pull-right hide'])
                    //     : '');
                        ?>
                    ...
                    <?= $form->field($model, 'parentID')->begin(); ?>
                        <?= Html::activeLabel($model, 'parentID') ?>

    <?= $this->render('manage-documents-tree', [
        'model' => $model,
        'isMultipleMode' => true,
    ]) ?>
                    <?= $form->field($model, 'parentID')->end(); ?>
...
<?php $this->registerJs("
    ...
    // added on 2020 april 17
    $('body').on('click', '#add-documents-button', function(e) {
        if( !$('#document-parentid').val() ){
            alert('" . Yii::t('admin', 'Please choose a folder.') . "');
            return false;
        }
    });
